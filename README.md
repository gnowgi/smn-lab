...
| **Q1 / Q1b** modulation & resolution | does modulation, not raw transduction, give resolution? | **modulation: yes** (audited) — without it, self/world resolution collapses as the body grows, and each zone must cancel *its own* motion (beats a head-only model 6.6–15.9×). **Density: not established** — the audit shows Q1b's rise with CAZ density is a 1/√N noise-averaging law that extra *raw transducers* satisfy just as well; discrimination-based re-test pre-registered (Q1c) |

...
!!! success "Status: wired — verdict PASS (both phases + capacity→performance)"
    The hypotheses, order parameters, and matched foils were fixed *before* the
    configuration-lattice sweep was run (preregistration below). Both phases are
    now wired — the Phase-A model + lattice sweep
    (`sweep_integrator_snapshot.py`, [first results](#first-results-phase-a)) and
    the [Phase-B embodied MuJoCo instance](#phase-b-embodied)
    (`integrator_snapshot_embodied.py`) — verdict **PASS**, with the
    capacity→performance link (`sweep_capacity_performance.py`) confirmed. The full
    build history is in `experiments/integrator_snapshot_DEVLOG.md`.
...
(split-circle = the appendicular-limit CAZ: one opponent pair = one DOF; light-blue = the coupling /
